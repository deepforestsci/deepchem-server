...
if TYPE_CHECKING:
    from deepchem_server.core.workflow.compute import ComputeWorkflow  # noqa: F401
...
def _init_datastore(profile_name: str, project_name: str, backend="local") -> DataStore:
    ...
    """
    if backend == "local":
        datastore: DataStore = DiskDataStore(
            profile_name=profile_name, project_name=project_name, basedir=DATA_DIR
        )
    else:
...
def run_job(profile_name: str, project_name: str, program: Dict, backend: str = "local"):
    ...
    """
    if backend == "local":
        # Lazy import to avoid circular dependency and heavy imports at module load
        from deepchem_server.core.workflow.compute import ComputeWorkflow  # noqa: F401

        logger.info("beginning")
        datastore: DataStore = _init_datastore(
...
def _upload_data(
    profile_name, project_name, datastore_filename, contents, data_card, backend="local"
):
    """
    ...
    dataset_address = datastore.upload_data(
        datastore_filename=datastore_filename, filename=temppath, card=data_card
    )

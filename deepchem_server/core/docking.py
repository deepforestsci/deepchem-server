...
def _upload_complexes_to_datastore(complexes: list, output: str, datastore, tmp_dir: str) -> str:
    ...
    ----------
    complexes : list
        List of complex file paths generated by VINA
    output : str
        Base output name for the complexes
    datastore : DataStore
    ...
    for i, complex_item in enumerate(complexes):
        # Handle both file paths and complex objects
        if isinstance(complex_item, str):
            # It's a file path
            complex_path = complex_item
        else:
    ...
    if complex_addresses:
        ...
        first_complex = complex_addresses[0]
        dir_address = first_complex.rsplit('/', 1)[0] if '/' in first_complex else complexes_dir
        return dir_address
    else:
        # If no complexes were uploaded, return empty directory address
        log_progress('docking', 85, 'Warning: No complexes were uploaded to datastore')
        return f"{output}_complexes"
...
def generate_pose(
    ...
    str
        ...
        - complexes_address: Address of the complexes directory
    ...
    ...     complex = datastore.get(complex_address, kind='data')
    ...     print(f"Complex {i}: {complex.n_atoms} atoms")
    ...
    try:
        ...
        with tempdir as tmp:
            ...
            # Upload complexes to datastore
            complexes_address = _upload_complexes_to_datastore(complexes, output, datastore, tmp)
            ...
            results = {
                ...
                'complexes_address': complexes_address,

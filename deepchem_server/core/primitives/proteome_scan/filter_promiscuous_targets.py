...
def filter_promiscuous_targets(
    ...
    log_progress("filter_promiscuous_targets", 5,
                 "downloading scan result CSVs from datastore")
    ...
    for address in scan_result_addresses:
        ...
        if not isinstance(df, pd.DataFrame):
            raise ValueError(
                f"Expected CSV at address {address}, got {type(df)}")
        if "gene_name" not in df.columns:
            raise KeyError(
                f"CSV at {address} is missing required 'gene_name' column")
        filename = os.path.basename(address.split("://")[-1])
        dfs[filename] = df

    log_progress(
        "filter_promiscuous_targets", 20,
        "identifying promiscuous targets for each threshold")
    ...
    for threshold_pair in thresholds:
        ...
        gene_counts = (pd.concat(top_slices)["gene_name"]
                       .value_counts().rename_axis("gene_name")
                       .reset_index(name="occurrence in top"))
        promiscuous_genes: List[str] = gene_counts[
            gene_counts["occurrence in top"] >= n]["gene_name"].tolist()

        key = f"{m}%_{n}"
        promiscuous_targets[key] = promiscuous_genes

    log_progress("filter_promiscuous_targets", 40,
                 "uploading promiscuous targets JSON")

    promiscuous_targets_json = json.dumps(promiscuous_targets, indent=2)
    promiscuous_card = DataCard(address="", file_type="json",
                                data_type="json")
    ...
    log_progress(
        "filter_promiscuous_targets", 50,
        "filtering and uploading per-threshold CSVs")
    ...
    for i, threshold_pair in enumerate(thresholds):
        ...
        for filename, df in dfs.items():
            ...
            csv_card = DataCard(address="", file_type="csv",
                                data_type="pandas.DataFrame")
            filtered_address = datastore.upload_data_from_memory(
                filtered_df, csv_name, csv_card)
            filtered_addresses[filename] = filtered_address

        filtered_results[key] = filtered_addresses
        progress = 50 + int(40 * (i + 1) / n_thresholds)
        log_progress(
            "filter_promiscuous_targets", progress,
            f"uploaded filtered CSVs for threshold {key}")

    log_progress("filter_promiscuous_targets", 90,
                 "uploading results summary")
    ...
    log_progress(
        "filter_promiscuous_targets", 100,
        "filter_promiscuous_targets completed successfully")

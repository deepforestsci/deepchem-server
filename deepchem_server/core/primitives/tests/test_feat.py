...
def test_featurize_multicore(disk_datastore):
    ...
    """
    config.set_datastore(disk_datastore)
    dataset_path = os.path.join(ASSETS_PATH, "zinc5k.csv")
...
def test_featurize_multicore_restart(disk_datastore):
    ...
    config.set_datastore(disk_datastore)
    dataset_path = os.path.join(ASSETS_PATH, "zinc5k.csv")
    partial_checkpoints_dir_path = os.path.join(ASSETS_PATH, "feat_test.partial")
    df = pd.read_csv(dataset_path)
    card = DataCard(address="", file_type="csv", data_type="pandas.DataFrame")
    data_address = disk_datastore.upload_data_from_memory(df, "zinc.csv", card)
    ...
    assert ("deepchem://" + disk_datastore.storage_loc + "/" + checkpoint_output_key + "/_checkpoint/part_0_of_3.cdc"
            in disk_datastore.list_data().split("\n"))
    assert ("deepchem://" + disk_datastore.storage_loc + "/" + checkpoint_output_key + "/_checkpoint/part_1_of_3.cdc"
            in disk_datastore.list_data().split("\n"))
...
def test_featurize_multicore_sdf(disk_datastore):
    ...
    """
    config.set_datastore(disk_datastore)
    dataset_path = os.path.join(ASSETS_PATH, "membrane_permeability.sdf")
...
def test_featurize_sdf(disk_datastore):
    ...
    sdf_test_file = os.path.join(ASSETS_PATH, "membrane_permeability.sdf")

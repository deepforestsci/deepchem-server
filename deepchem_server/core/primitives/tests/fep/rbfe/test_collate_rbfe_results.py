...
# Path to test assets - moved to primitives/tests/assets
...
def test_process_input_files_with_invalid_file(caplog):
    ...
    assert re.match(
        r".*Could not process(.+?){}.*$".format(re.escape("rbfe-lig_ejm_48-lig_jmc_28.json")),
        caplog.text,
    )
...
def test_collate_rbfe_results(valid_uploaded_files_addresses):
    ...
    output_datastore_address = collate_rbfe_results(
        valid_uploaded_files_addresses,
        "lig_ejm_48",
        "-9.00 kcal/mol",
        "0.00 kcal/mol",
        output_file_name,

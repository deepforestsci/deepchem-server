...
def featurize_part(
    ...
    datastore_basedir: Optional[str],
) -> None:
    ...
    datastore_profile : str
        Profile name for datastore initialization in child process.
    datastore_project : str
        Project name for datastore initialization in child process.
    datastore_basedir : str, optional
        Base directory for datastore initialization in child process.
    ...
    NotImplementedError
        If the file type is not supported for featurization.
    """
    # Initialize datastore in child process (multiprocessing doesn't inherit global state)
    from deepchem_server.core.common.datastore import DiskDataStore

    datastore = DiskDataStore(profile_name=datastore_profile, project_name=datastore_project, basedir=datastore_basedir)
...
def featurize_multi_core(
    ...
    """
    # Get datastore parameters to pass to child processes
    datastore = config.get_datastore()
    if datastore is None:
        raise ValueError("Datastore not set")
    datastore_profile = datastore.profile_name
    datastore_project = datastore.project_name
    datastore_basedir = datastore.basedir
    ...
    for dataset_path in dataset_paths:
        p = mp.Process(
            target=featurize_part,
            args=(
                main_dataset_address,
                dataset_path,
                file_type,
                feat,
                dataset_column,
...
def featurize(
    ...
    pattern = re.compile(rf"{_storage_loc}/{checkpoint_output_key}/_checkpoint/part_\d+_of_\d+\.cdc$")

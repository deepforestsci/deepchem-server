...
from deepchem_server.core.primitives.fep.rbfe.data_domain_classes.EdgeSimulationResult import (
...
def get_reference_ligand(ligand_mols: list[SmallMoleculeComponent]) -> int:
    ...
    df_summed = pd.DataFrame(np.sum(similarity_array, axis=1), columns=["sum"])  # type: ignore
...
def get_perturbation_network(
    ...
    **kwargs,
    ...
    if network_type in [
        ...
        assert (
            scorer is not None
        ), "A Scorer must be specified for Maximal and Minimal Spanning Networks"
    ...
    if network_type == NetworkPlanningConstants.PerturbationNetworkType.RADIAL:
        ...
        network_planner_params["ligands"] = (
            ligand_mols[:ref_idx] + ligand_mols[ref_idx + 1 :]
        )  # yapf: disable
...
def get_reference_ligand_index(
    ligand_mols: list[SmallMoleculeComponent], reference_ligand: Optional[str] = None
) -> int:
...
def setup_transformations(
    ...
    for edge in network.edges:
        ...
        runnable_edges.append(
            RunnableEdge(edge.componentA, edge.componentB, solvent_dag, complex_dag)
        )
...
def _is_dry_run_result_success(dry_run_result: dict) -> bool:
    ...
    if (
        ...
        and isinstance(dry_run_result, dict)
        and DEBUG in dry_run_result
        and dry_run_result[DEBUG][SAMPLER] is not None
    ):
        is_valid = True
...
def dry_run_edge(runnable_edge: RunnableEdge) -> EdgeSimulationResult:
    ...
    if _is_dry_run_result_success(solvent_dry_run_result) and _is_dry_run_result_success(
        complex_dry_run_result
    ):
...
def run_edge(
    runnable_edge: RunnableEdge, rbfe_transform: Type[RelativeHybridTopologyProtocol]
) -> EdgeSimulationResult:
    ...
    solvent_dag_results = execute_DAG(
        solvent_dag, scratch_basedir=solvent_path, shared_basedir=solvent_path, keep_scratch=True
    )

    # Execute the complex transformation
    complex_dag_results = execute_DAG(
        complex_dag, scratch_basedir=complex_path, shared_basedir=complex_path, keep_scratch=True
    )
    ...
    edgeSimulationResult = EdgeSimulationResult(
        componentA_name=componentA.name,
        componentB_name=componentB.name,
        is_dry_run=False,

...
def collate_rbfe_results(
    ...
    if (isinstance(reference_ligand_dg_value_uncertainty, str) and
            reference_ligand_dg_value_uncertainty.strip() == "None"):
    ...
    if not reference_ligand:
        ...
        assert (
            reference_ligand_dg_value is not None and reference_ligand_dg_value is not None
        ), "If the reference_ligand parameter is specified, reference_ligand_dg_value and reference_ligand_dg_value_uncertainty parameters MUST be specified."
...
def process_input_files(result_file_deepchem_addresses: list[str]):
    ...
    for deepchem_file_path in result_file_deepchem_addresses:
        try:
            ...
            config.get_datastore().download_object(file_name, temp_file_path)  # type: ignore
...
def calculate_dg_values(
    ...
    while True:
        ...
        for simulation_result in simulation_results:
            if (ligand_dg_values[simulation_result["componentA_name"]] is not None and
                    ligand_dg_values[simulation_result["componentB_name"]] is None):
            ...
            if (ligand_dg_values[simulation_result["componentA_name"]] is None and
                    ligand_dg_values[simulation_result["componentB_name"]] is not None):
                ligand_dg_values[simulation_result["componentA_name"]] = ExperimentalMeasurement(
                    ...
                    - pint.Quantity(simulation_result["edge_ddG"]),  # type: ignore
                    uncertainty=ligand_dg_values[simulation_result["componentB_name"]].uncertainty
                    + pint.Quantity(simulation_result["edge_ddG_uncertainty"]),  # type: ignore
                )  # yapf: disable
...
def get_result_dataframe(ligand_dg_values: dict):
    ...
    for key, value in ligand_dg_values.items():
        ...
        if value is None:
            ...
            dg_value = pint.Quantity(value.value).to(unit.kilocalorie / unit.mole)  # type: ignore
        ...
        result_dataframe.loc[len(result_dataframe)] = {
            ...
            "Uncertainty": uncertainty and uncertainty.round(4),  # type: ignore
        }
